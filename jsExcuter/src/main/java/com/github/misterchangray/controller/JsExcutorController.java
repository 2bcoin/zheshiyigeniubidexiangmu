...
public class JsExcutorController {
    ...
    //防止重复提交脚本
    public static String lastPostData = "";
    ...
    public ResultSet<String> shutdown(@RequestParam("data") String data) {
        ...
        for (RunningImage runningImage : JSExecutor.runningImage) {
            if (runningImage.isRunning() && runningImage.gettId().toString().equals(data)) {
                while (true) {
                    try {
                        runningImage.getThread().interrupt();
                        runningImage.getThread().join();
                        if(false == runningImage.getThread().isAlive()) {
    ...
    @ResponseBody
    @RequestMapping(method = RequestMethod.GET, value = "/shutdownnow")
    public ResultSet<String> shutdownnow(@RequestParam("data") String data) {
        data = getParam(data);
        if (null == data)
            return ResultSet.build(ResultEnum.VALIDATEPARAMETER);
        long start = System.currentTimeMillis();
    ...
    public ResultSet<ScriptDto> info(@RequestParam("id") String id) {
        ...
        for (RunningImage runningImage : JSExecutor.runningImage) {
            if (runningImage.isRunning() &&
                    runningImage.getScript().getIpAndScriptId().equals(id)) {
                m.put("title", runningImage.getScript().getTitle());
                m.put("ipAndScriptName", runningImage.getScript().getIpAndScriptName());
                m.put("running", runningImage.isRunning());
    ...
    public ResultSet<String> log(@RequestParam(value = "page", required = false) Integer page,
        ...
        if (null == page) page = 1;
        return ResultSet.build(ResultEnum.SUCCESS).setData(h2DataBaseCache.log(tId, limit, page)).setPageInfo(new PageInfo(page, (long) h2DataBaseCache.logCount(tId), limit));
    }
}
